...
export default function ShopScreen({ save, onPack, onClaimCharges, onWatchAd }) {
  ...
  const RARITY_INFO = [
    ...
    { r:'L', label:`≥ ${(VIEW_THRESHOLDS.L/1000).toFixed(0)}k views/month`, note:'World-famous trains' },
    { r:'E', label:`${(VIEW_THRESHOLDS.E/1000).toFixed(0)}k–${(VIEW_THRESHOLDS.L/1000).toFixed(0)}k/month`, note:'Well-known to enthusiasts' },
    { r:'R', label:`${(VIEW_THRESHOLDS.R/1000).toFixed(0)}k–${(VIEW_THRESHOLDS.E/1000).toFixed(0)}k/month`, note:'Notable regional trains' },
    { r:'C', label:`${VIEW_THRESHOLDS.MYTHIC_MAX}–${(VIEW_THRESHOLDS.R/1000).toFixed(0)}k/month`, note:'Everyday locomotives' },
  ];
  ...
  return (
    <div style={{ padding:'18px 14px', maxWidth:480, margin:'0 auto' }}>
      ...
      <div style={{ background:'#0c1825', border:'1px solid rgba(201,168,51,0.18)', borderRadius:9, padding:'13px', marginBottom:13, textAlign:'center' }}>
        ...
        <div style={{ fontSize:8, color:'#1e3050', fontFamily:'monospace', marginTop:3 }}>CURRENT BALANCE · {PACK_COST} TICKET PER PACK</div>
      </div>
      ...
      <div style={{ background:'#0c1825', border:'1px solid rgba(201,168,51,0.13)', borderRadius:9, padding:'14px', marginBottom:11 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:11 }}>
          <div>
            ...
            <div style={{ fontSize:7.5, color:'#1e3050', fontFamily:'monospace', marginTop:2 }}>+{TIMER_TICKETS} every 5 min · up to {TIMER_MAX_CHARGES} stored</div>
          </div>
        ...
        <button onClick={charges > 0 ? onClaimCharges : undefined} style={{
          ...
          {charges > 0
            ? `COLLECT ${charges} CHARGE${charges > 1 ? 'S' : ''} (+${charges * TIMER_TICKETS} TICKET${charges > 1 ? 'S' : ''})`
      ...
      <div style={{ background:'#0c1825', border:'1px solid rgba(79,168,232,0.18)', borderRadius:9, padding:'14px', marginBottom:13 }}>
        ...
        <button onClick={!cooldown ? () => setShowAd(true) : undefined} style={{
          ...
          {cooldown ? `NEXT AD READY IN ${fmtMs(msAd)}` : `▶ WATCH AD FOR +${AD_TICKETS} TICKETS`}
        </button>
      </div>

      {/* Open pack */}
      <div style={{ background:'linear-gradient(160deg,#0d1f35,#060f1c)', border:'1px solid rgba(201,168,51,0.28)',
        borderRadius:12, padding:'20px', textAlign:'center', boxShadow:'0 0 28px rgba(201,168,51,0.06)', marginBottom:16 }}>
        ...
        <div style={{ fontSize:8.5, color:'#1e3a5a', fontFamily:'monospace', marginBottom:16 }}>5 REAL LOCOMOTIVE CARDS · WIKIPEDIA INFO &amp; PHOTOS</div>
        <button className="btn" onClick={can ? onPack : undefined} style={{
          ...
          {can ? `🎫 OPEN — ${PACK_COST} TICKET` : 'NOT ENOUGH TICKETS'}
        </button>
      </div>

      {/* Rarity guide */}
      <div style={{ background:'#0c1825', border:'1px solid rgba(201,168,51,0.09)', borderRadius:9, padding:'13px' }}>
        <div style={{ fontSize:8.5, color:'#c9a833', fontFamily:'monospace', letterSpacing:'.15em', marginBottom:8 }}>HOW RARITY WORKS</div>
        <p style={{ fontSize:8, color:'#1e3050', fontFamily:'monospace', margin:'0 0 10px', lineHeight:1.75 }}>
          Rarity comes from real Wikipedia page views. ✦ Mythic means the train is almost completely unknown — rarer than Legendary because most people have never heard of it. Legendary means the train is globally iconic.
        </p>
        {RARITY_INFO.map(({ r, label, note }) => {
          const rs = RARITY[r];
          return (
            <div key={r} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
              padding:'6px 0', borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                {r === 'M' && <span style={{ fontSize:8, color:rs.color }}>✦</span>}
                <span style={{ fontSize:8.5, color:rs.color, fontFamily:'monospace', fontWeight:700, minWidth:60 }}>{rs.name}</span>
                <span style={{ fontSize:7.5, color:'#1a3050', fontFamily:'monospace' }}>{note}</span>
              </div>
              <span style={{ fontSize:8, color:'#1e3050', fontFamily:'monospace', whiteSpace:'nowrap', marginLeft:6 }}>{label}</span>
            </div>
          );
        })}
      </div>
