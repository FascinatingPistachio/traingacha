...
export default function HomeScreen({ save, onDaily, onPack, goShop }) {
  ...
  return (
    <div style={{ padding: '18px 14px', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 12 }}>
        {[
          ...
          <div key={s.label} style={{
            ...
            <div style={{ fontSize: 18, color: '#c9a833', fontFamily: 'monospace', fontWeight: 700 }}>
              {s.icon} {s.val}
            </div>
            <div style={{ fontSize: 7.5, color: '#1e3050', marginTop: 3, fontFamily: 'monospace' }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      {/* Daily bonus */}
      <button
        onClick={canClaim ? onDaily : undefined}
        style={{
      ...
      >
        {canClaim
          ? `🎁 CLAIM DAILY BONUS (+${DAILY_BONUS} TICKETS)`
      ...
      <button
        className="btn"
        onClick={save.tickets >= PACK_COST ? onPack : goShop}
        style={{
          ...
          borderRadius: 9,
          color: save.tickets >= PACK_COST ? '#4fa8e8' : '#1e3050',
          fontSize: 12, fontFamily: 'monospace', cursor: 'pointer',
          letterSpacing: '.07em', transition: 'all 0.2s',
        }}
      >
        {save.tickets >= PACK_COST
          ? `🎴 OPEN A PACK — ${PACK_COST} TICKETS`
          : `🎴 GO TO PACK SHOP (need ${PACK_COST - save.tickets} more)`}
      </button>
    </div>
  );
}
