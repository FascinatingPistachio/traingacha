...
function EmptyState() {
  return (
    <div style={{ textAlign:'center', padding:'60px 20px', color:'#1e3050', fontFamily:'monospace', fontSize:11, lineHeight:1.8 }}>
...
export default function CollectionScreen({ collection, favourites = new Set(), onToggleFav }) {
  ...
  const filtered = entries
    ...
    .sort((a, b) => {
      ...
      if (sort === 'favs')     return (favourites.has(b.id)?1:0) - (favourites.has(a.id)?1:0);
  ...
  return (
    <div style={{ padding:'14px 10px', maxWidth:540, margin:'0 auto' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:14 }}>
        <h2 style={{ color:'#c9a833', margin:'0 0 3px', fontSize:13, fontFamily:'monospace', letterSpacing:'.2em' }}>COLLECTION</h2>
        <p style={{ fontSize:8, color:'#1a3050', margin:0, fontFamily:'monospace' }}>{entries.length} UNIQUE CARDS · TAP TO EXPAND</p>
      </div>
      ...
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search cards…"
        style={{ width:'100%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(201,168,51,0.15)',
          borderRadius:7, padding:'8px 12px', color:'#e8e0d0', fontSize:11.5, outline:'none',
          fontFamily:'monospace', marginBottom:10 }} />

      {/* Filter row */}
      <div style={{ display:'flex', gap:5, marginBottom:10, flexWrap:'wrap', justifyContent:'center' }}>
        {['ALL','M','L','E','R','C'].map(r => {
          const rs = r==='ALL' ? {color:'#4fa8e8',bg:'#07182a',border:'rgba(79,168,232,0.4)'} : RARITY[r];
          ...
          return (
            <button key={r} onClick={()=>setFilter(r)} style={{
              ...
              borderRadius:5, color:active?rs.color:'#1e3a5a',
              fontSize:8, cursor:'pointer', fontFamily:'monospace', letterSpacing:'.1em',
            }}>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:14, justifyContent:'center', flexWrap:'wrap' }}>
        <select value={sort} onChange={e=>setSort(e.target.value)} style={{
          padding:'4px 8px', background:'#0c1825', border:'1px solid rgba(255,255,255,0.07)',
          borderRadius:5, color:'#2a4a6a', fontSize:8, cursor:'pointer', fontFamily:'monospace', outline:'none',
        }}>
          <option value="rarity">BY RARITY</option>
          <option value="views">MOST FAMOUS</option>
          <option value="obscure">MOST OBSCURE</option>
          <option value="newest">NEWEST FIRST</option>
          <option value="name">A–Z</option>
          <option value="favs">FAVOURITES FIRST</option>
          <option value="char">CHARACTERS FIRST</option>
        </select>
        ...
        <button onClick={()=>setShowFavsOnly(!showFavsOnly)} style={{
          ...
          borderRadius:5, color:showFavsOnly?'#ff6b6b':'#1e3a5a',
      ...
      {entries.length === 0 ? <EmptyState /> : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:10 }}>
          ...
          {filtered.length === 0 && (
            <p style={{ gridColumn:'1/-1', textAlign:'center', color:'#1e3050', fontFamily:'monospace', fontSize:10, padding:'30px 0' }}>
              No cards match.
            </p>
          )}
        </div>
